...
const utilService = require('../domain/utilService');
...
const crearFamiliar = (request, response) => {
    ...
    try {
        ...
        if (esRegistroParaRelacionar) {
            ...
            utilerias.findCorreoPadre(p.correo,p.id_empresa)
                .then((encontrado) => {
...
const enviarClaveFamiliar = async (idFamiliar) => {

    try {

        const nuevoPassword = await utilService.generarRandomPassword();
        await familiarService.modificarPassword(idFamiliar,nuevoPassword);                
         mailService.enviarCorreoClaveFamiliar(idFamiliar,nuevoPassword);
        /*mailService.enviarCorreoClaveFamiliar(
            familiar.correo,
            "Contraseña",
            {
...
const createFamiliar = (id_alumno, familiar, genero) => {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            pool.query(
                "  INSERT INTO co_familiar(nombre,telefono,fecha_nacimiento,correo,password,celular,religion,nota_celebracion_dia,genero,cat_genero,co_empresa)" +
                ...
                [
                    ...
                    genero,p.cat_genero,p.id_empresa
                ],
                (error, results) => {
                    if (error) {
...
const getParams = (body) => {
    const parametros = {
        ...
        cat_genero,id_sucursal,id_empresa
    } = body;
