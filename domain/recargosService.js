    ...
    if (existeValorArray(listaSucursales)){
        for (let index in listaSucursales) {
            ...
            if (!isEmptyOrNull(sucursal)) {
                ...
                //obtener el cargo correspondiente a la empresa 
                const cargoExtraMensualidad = await cargoService.getCargoExtraMensualidadEmpresa(sucursal.id_empresa);

                if(!cargoExtraMensualidad || cargoService == null){
                    console.log("No se generaron los cargos extras por que no existe el cargo extra en el catalogo");
